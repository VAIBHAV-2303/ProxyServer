...
class Server():
	...
	def __init__(self, config, blocked,credentials):
		...
		while 1:
			...
			th = threading.Thread(name = self._getClientName(client_address), target = self.proxy_thread, args = (clientSocket, config, blocked,credentials))
			th.setDaemon(True)
			th.start()

		self.serverSocket.close()

	def proxy_thread(self, clientSocket, config, blocked,credentials):
		...
		try:
			temp_data = str_req.split()[str_req.split().index("Basic")+1].split("\\r")[0] 
			decoded = base64.b64decode(temp_data).decode("utf-8")
...
server = Server(config, blocked,credentials)